...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!response.ok) { const detail = (await response.json()).detail; throw new Error(Array.isArray(detail) ? detail.map(item => `${item.loc.at(-1)}: ${item.msg}`).join(', ') : detail ?? '요청에 실패했습니다.') }
